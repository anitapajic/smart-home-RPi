...
def run_dht1(settings, threads, stop_event, print_lock):
    ...
    if settings['simulated']:
        ...
        dht1_thread = threading.Thread(target=run_dht_loop, args=(dht, 2, dht_callback, stop_event, print_lock, dht))
        dht1_thread.start()
        threads.append(dht1_thread)
...
def run_dht2(settings, threads, stop_event, print_lock):
    ...
    if settings['simulated']:
        ...
        dht1_thread = threading.Thread(target=run_dht_simulator, args=(2, dht_callback, stop_event, print_lock, dht_name))
        dht1_thread.start()
        threads.append(dht1_thread)
        #print("Dht1 sumilator started")
    else:
        ...
        dht = DHT(settings['pin'])
        dht1_thread = threading.Thread(target=run_dht_loop, args=(dht, 2, dht_callback, stop_event, print_lock, dht_name))
        dht1_thread.start()
        threads.append(dht1_thread)
